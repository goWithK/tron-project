...
import { EtherScanAPI } from "../apis/etherscan.api";
import * as dotenv from 'dotenv';
dotenv.config();

const web3 = new Web3(new Web3.providers.HttpProvider(`${process.env.QN_ENDPOINT_ETH}`))
...
export async function getPairSushiV2(contractAddress: string) {
    ...
    try {
        ...
        pairAddress = await contract.methods.getPair(contractAddress, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2').call();
    } catch (e) {
        console.error(e)
...
export async function getPairUniV3(contractAddress: string) {
    ...
    try {
        ...
        pairAddress = await contract.methods.getPool(contractAddress, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', 100).call();
    } catch (e) {
        console.error(e)
...
export async function getPairUniV2(contractAddress: string) {
    ...
    try {
        let contract = new web3.eth.Contract(abiGetPair, '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f');
        pairAddress = await contract.methods.getPair(contractAddress, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2').call();
    } catch (e) {
        console.error(e)
...
export async function getFirstReceiptPair(pairAddress: any) {
    ...
    try {
        ...
        const txHash = await EtherScanAPI.getFirstInternalTxn(currentBlock, pairAddress);
...
export async function getPairAddress(contractAddress: string, deployerAddress: string) {
    ...
    try {
        if (pairAddressUniV2) {
            let firstITxnUniV2hash = await EtherScanAPI.getFirstInternalTxn(currentBlock, pairAddressUniV2);
    ...
    try {
        if (pairAddressUniV3) {
            let firstITxnUniV3hash = await EtherScanAPI.getFirstInternalTxn(currentBlock, pairAddressUniV3);
    ...
    try {
        if (pairAddressSushi) {
            let firstITxnSushihash = await EtherScanAPI.getFirstInternalTxn(currentBlock, pairAddressSushi);
    ...
    try {
        if (pairAddressSB) {
            let firstITxnSBhash = await EtherScanAPI.getFirstInternalTxn(currentBlock, pairAddressSB);
            if (firstITxnSBhash) {
...
export async function getInitLPbyDeployer(deployerAddress: string) {
    ...
    try {
        const currentBlock = await web3.eth.getBlockNumber().then(value => { return Number(value) });
        resp = await EtherScanAPI.getTxnbyAddress(currentBlock, deployerAddress);
    } catch (e) {
        console.error(e)
...
export async function getCAbyDeployer(deployerAddress: string) {
    ...
    try {
        const currentBlock = await web3.eth.getBlockNumber().then(value => { return Number(value) });
        txns = await EtherScanAPI.getTxnbyAddress(currentBlock, deployerAddress);
    } catch (e) {
        console.error(e)
